...
function init() {
	...
	kpiAttr.data = tcCore.getTopWindowParam();
	initControls();
	initGrid();
...
function initControls() {
	...
	if(kpiAttr.data.isKpi==1){
		$("#isKpi").get(0).checked = true;
	}
	...
	tcCore.get({
		...
		success : function(data) {
			...
			$("#varId").combobox("setValue", kpiAttr.data.varId);
...
function getQutota(){
	tcCore.get({
		url : "jobQuotaDS/getAllRuleVarList.ssm",
		success : function(data) {
			$("#varId").combobox("loadData", data);
			...
			.datagrid(
					{
						...
						columns : [ [
								...
								{
									field : 'kpiInput',
									//简化版输入的时候，客户要求把基数和比例的乘积作为比例，也就是原有的比例都为1即可
									title : '比例',
									width : 100,
									editor : {
										type : 'textbox'
...
function saveKpi() {
	...
	var checked = $("#isKpi").get(0).checked;
	if(checked){
		data.isKpi = 1;
	}else{
		data.isKpi = 0;
	...
	tcCore.post({
		url : "jobQuotaDS/saveKpiGradeListSimple.ssm",
		data : {
