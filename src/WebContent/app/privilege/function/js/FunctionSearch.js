...
function initMoreConditionBtnClick(){
	$("#moreConditionBtn").click(function(){
		if($("#functionTable").data().opened){
			$("#functionTable").children().children().each(function(i,o){
				if(i!=0){
					$(o).css("display","none");
				}
			});
			$("#functionTable").data().opened = false;
			$("#moreConditionBtn").parent().removeAttr("vAlign");
		}else{
			$("#functionTable").children().children().each(function(i,o){
				$(o).css("display","");
			});
			$("#functionTable").data().opened = true;
...
function initGrid(){
	...
	$('#functionGridDiv').css({
		height:window.innerHeight-$("#functionForm").innerHeight()-$("#operbtn").innerHeight()-30-20-30
	});
	...
	$('#functionGrid').datagrid({
	    ...
	    columns:[[
	 	    	 {field:'operation_custom',title:'操作',width:60,align:"center",formatter:function(value,row,index){
			  	  		return "<a title='查看' class='grid_operation_btn grid_operation_btn-view'  onclick=viewData(this,'"+row.functionId+"')></a>&nbsp;"+
			  	  		"<a title='编辑' class='grid_operation_btn grid_operation_btn-edit' onclick=editData(this,'"+row.functionId+"')></a>&nbsp;" +
			  	  				"<a title='删除' class='grid_operation_btn grid_operation_btn-delete'  onclick=deleteData(this,'"+row.functionId+"')></a>";
			  	  	}},
,
		    	  	 	{field:'remark',title:'描述',width:100}
,
		    	  	 	{field:'uri',title:'资源标识',width:100}
,
...
function queryFunction(){
	var param = tcCore.getFormData("functionForm");
	var param1 =tcCore.getFormData("functionFormMoreCondition"); 
	var p = {};
	copyProerty(p,param);
...
function sendQuery(param){
	...
	// 计算起始，和截至数据
	tcCore.post({
		url:"functionDS/queryForListPage.ssm",
		data:{
			params:param,
			size:pager.pageSize,
...
function viewData(a,id){
	// debugger;
	tcCore.openWindowOnTop("FunctionView.jsp?functionId="+id, null, a, function(result){
		// 关闭窗口的回调
	}, {
		title:"查看Function"
	});
 ...
 */
function addData(){
	tcCore.openWindowOnTop("FunctionEdit.jsp", null, null, function(result){
		// 关闭窗口的回调
...
function editData(a,id){
	// 打开编辑窗口
	tcCore.openWindowOnTop("FunctionEdit.jsp?functionId="+id, null, a, function(result){
		// 关闭窗口的回调
		operaCallBack(result);
	}, {
		title:"编辑Function"
...
function deleteDataBtnClick(){
	...
	if(data==null){
		...
		deleteData(null,data.functionId);
	}
}
