...
function getMenu() {
	tcCore.get({
		url : "wxMenuDS/getMenuTree.ssm?noDisable=0",
 ...
 * 微信菜单同步
 */
function weixinMenuSynchronous(){
	tcCore.post({
		url:"wxMenuDS/weixinMenuSynchronous.ssm",
		data:{
		},
		success:function(data){
			if(data.code==0)
			{
				...
				$.messager.alert('提示','生成失败!<br>'+data.text,'info');
			}
	...
	if(rows.length>0){
		...
		top.$.messager.confirm('系统提示', '确认要删除该菜单信息吗', function(r) {
			if (r) {
				if (menuId) {
					tcCore.post({
						url : "wxMenuDS/deleteMenuByPk.ssm",
						data : {
							menuId : menuId
						},
						success : function(data) {
