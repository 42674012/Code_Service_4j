...
//                }
                var imgsrc="http://"+goodsAttr.imgserURL+node.id;
                return arr.push({
                    _src: imgsrc,
                    src: imgsrc
                });
        ...
        close: function ($img) {
            ...
            $img.css({
            	...
            	var fileid=$img.context.id;
            	//获得附件服务器的url
            	tcCore.get({
            		url:"fileOptions/fileDelete.ssm?fileId="+fileid,
        ...
        callback: function (editor, $w, url, state) {
            if (state == "SUCCESS") {
                //显示图片计数+1
            	editor.options.imagePath="http://"+goodsAttr.imgserURL;
            	var showurl=url.substring(0,url.lastIndexOf("."))+"_small"+url.substring(url.lastIndexOf("."),url.length);
                Upload.showCount++;
                var $img = $("<img id='"+url+"' src='" + editor.options.imagePath + showurl + "' class='edui-image-pic' />"),
